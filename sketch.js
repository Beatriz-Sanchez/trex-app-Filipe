...
function setup(){
  ...
  //cria solo invisível
  soloInvisivel = createSprite(300,windowHeight-20,600,40);
  soloInvisivel.visible = false;
  
  //cria sprite do T-Rex
  trex = createSprite(60,windowHeight - 90,20,50);
  trex.scale = 0.1;
  trex.x = 50;
...
function draw(){
  ...
  if(estadoJogo === JOGAR){
    ...
    if(keyDown('space') && trex.y>windowHeight-80 || touches.length > 0 && trex.y>windowHeight-80){
      trex.velocityY = -16; 
      somSalto.play();
      touches= [];
    }
...
function gerarNuvens(){
  ...
  if(frameCount %100 === 0){
    ...
    nuvem.y = Math.round(random(30,windowHeight/2-10));
